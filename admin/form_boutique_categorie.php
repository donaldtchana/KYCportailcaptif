...
if(!empty($_GET['id'])){
    ...
    $query = $db->query('SELECT * FROM categorie_boutiques WHERE id=:id  LIMIT 1', array("id"=>$_GET['id']));
    if($query->rowCount() > 0){
        $update=true;
        $action='Modifier';
    }
...
$page_name=$action.' une catégorie de boutique';
?>
<!DOCTYPE html>
                ...
                <div class="col-md-6 col-8 align-self-center">
                    ...
                    <div class="d-flex align-items-center">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="cat_boutiques">Retour</a></li>
...
if(isset($_POST['save'])){
    ...
    if( !empty($input->post('id'))){
        ...
        $exec= $db->update("categorie_boutiques",$values,array("id"=>$input->post('id')));
        $msg='La catégorie a été modifiée avec succes.';
        $action='La catégorie de boutique #'.$input->post('id').' '.$input->post('nom').' a été modifiée.';
    }else{
        $exec= $db->insert("categorie_boutiques",$values);
        $msg='La catégorie a été ajoutée avec succes.';
        $action='La catégorie de boutique #'.$db->lastInsertId().' '.$input->post('nom').' a été ajoutée.';
